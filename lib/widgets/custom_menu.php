...
class MeerkatMenuWidget extends MeerkatWidget {
    ...
    public function __construct() {
        ...
        if ($menus) {
            ...
            $this->fields = array('error' => array('type'  => 'message',
                                                   'label' => 'No custom menus!&nbsp; <a href="' . get_home_url() . '/wp-admin/nav-menus.php">Create one</a>')
            );
        }
    }
    ...
    public function widget($args, $instance) {
        ...
        $menu = new \Timber\Menu($instance['nav_menu']);
        $menu = \Timber\Timber::fetch('modules/navigation.twig', array(
            'nav_obj'        => $menu,
            'menu_name'      => $menu->name,
            'menu_id'        => $menu->slug,
            'menu_class'     => 'menu vertical collapse',// . $layout . $toggle,
            'toggle'         => true,
            'title'          => __('Menu'),
            'load_collapsed' => true
        ));
...
// register widget
add_action( 'widgets_init', function(){ register_widget( "MeerkatMenuWidget" ); });