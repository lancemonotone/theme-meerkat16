	...
	function handle_shortcode( $atts, $content = null ) {
		...
		foreach ( $attachments as $attachment ) {
			...
			if ( $type == 'filmstrip' ){
		 		 ...
		 		 $link_html .= 'rel="meerkat-image-group" href="' . $item['href'] . '"';
			}
		 	if ( $item['caption'] ){
	       	    $link_html .= ' caption="' . $item['caption'] . '"';
			}
			$link_html .= '>';

			if (( $type == 'grid' && $link != 'none' ) || $type != 'grid' ){
		       $html['link'] .= $link_html;
			}

			//---- IMG ---- //			
			if ( $type == 'slideshow' || $type == 'filmstrip' ) { $item['alt'] = $item['caption']; }
			$html['img'] = '<img alt="' . $item['alt'] . '" src="' . $item['src'] . '"';
			if ( $type == 'filmstrip' ){
		       ...
		       //$html['img'] .= ' style="width:' . $item['width'] . 'px; height:' . $item['height'] . 'px;"';
		    }
			...
			if ( $caption && $type != 'slideshow' ){	
				...
				$html['caption'] = '<div class="gallery-caption" style="width:' . $adjusted_w . 'px;">' . $item['caption'] . '</div>';			
			...
			if ( $type == 'slideshow' ){
				...
				$all_items .= $html['link'] . $html['img'] . '</a>';
			}
			/* -- UNDER DEVELOPMENT -- 
			else if ( $tooltip ){
			    $all_items .= '<li class="gallery-icon icon-' . $size . ' tooltip-container">';
				$all_items .= $html['link'] . $html['img'] . '</a>';
				if ( $item['caption'] ){
				   $all_items .= '<div class="tooltip">' . $item['caption'] . '</div></li>';
				}
			}*/			
			else {
				$all_items .= '<li class="gallery-icon icon-' . $size . '">' . $html['link'] . $html['img'] . '</a>';
				$all_items .= $html['caption'] . '</li>'; 
			}
		}
