<?php
require_once(WMS_EXT_LIB . '/fpdf/fpdf.php');

add_action('flexiform_update', 'wms_update_office_directory_pdf');
function wms_update_office_directory_pdf($schemaId) {
    // This operation may occur in a non-www context (ie, network), so only update pdf in
    // HR site files dir: hr.williams.edu = site id 27
    $upload_dir_info = wp_upload_dir();
    $upload_dir             = dirname(dirname($upload_dir_info['basedir'])) . '/27/files/';
    ...
    $pdf->Output($upload_dir . 'directory.pdf', 'F');
...
class PDF extends FPDF {
    ...
    function __construct($orientation = 'P', $unit = 'mm', $size = 'A4', $args = array()) {
...
function theme_display_flexiform_data($data_array) {
    ...
    switch ($dataset) {
        ...
        case 'Honorary Degrees':
            ...
            return '<div class="honorary-degree">' . '<table class="wms-dept-info"><tr><td colspan="2">' . $search . '</td><td colspan="2""><div class="pagination">' . $page_links . '</div></td></tr>' . $guts . "</table></div>\n<!-- end honorary-degree dev -->\n";
            break;
    } // end switch
