...
require(['jquery', 'jquery_ui'], function($){
     ...
     */
    !function (o) {
        o.widget("custom.tooltipX", o.ui.tooltip, {
            options: {autoShow: !0, autoHide: !0}, _create: function () {
                this._super(), this.options.autoShow || this._off(this.element, "mouseover focusin")
            }, _open: function (o, t, i) {
                this._superApply(arguments), this.options.autoHide || this._off(t, "mouseleave focusout")
            }
        })
    }(jQuery);
    ...
    (function ($) {
        ...
        (function (container, context) {
			...
			$('#site-navigation .nav-link').each( function( index, value ) {
            ...
            $menuToggle.on('click.twentysixteen', function () {
                ...
                function closeDrawer(){
                    $targetParent.slideUp(300, function () {
                        $menuToggle.removeClass('toggled-on auto-opened');
...
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define('detect_swipe',['jquery'], factory);
...
(function(root) {
define("featherlight_src", ["jquery","detect_swipe"], function() {
  return (function() {
/**
...
define('featherlight',['jquery','featherlight_src'], function($){
...
// @todo: revisit making this RequireJS module
 define('main',['jquery', 'featherlight'], function ($) {
    jQuery(document).ready(function ($) {
        ...
        function set_button_tab($target) {
            ...
            if ($target) {
                ...
                $target_div = $('.button-tabs').find('div[data-target="' + $target + '"]');
        ...
        $('.category-filter').click(function (e) {
            $desired_cat = $(this).attr('data-slug');
            $show_all = $(this).hasClass('show-all');
        ...
        $('.cycle-container .cycle-control').click(function (e) {
            // get current state
            $paused = $(this).parent().find('.meerkat-image-gallery').is('.cycle-paused');
            if ($paused) {
        ...
        $gallery_widgets = $('#sidebar .widget_meerkat_gallery');
        ...
        function mk_gallery_widget_size() {
            ...
            $gallery_widgets.each(function () {
                $gallery_container = $(this).find('.meerkat-gallery');
                $gallery_container_limit = $gallery_container.attr('limit');
                $gallery_widget_w = $(this).width() - 1;
                $gall_item_w = ($gallery_widget_w - ($pad * 3)) / 2;
                $gall_item_w = Math.floor($gall_item_w) - $border;
                $(this).find('img').each(function () {
                    $(this).width($gall_item_w + 'px');
                    $(this).height($gall_item_w + 'px');
                });
                $rows = $gallery_container_limit / 2;
                $h = ($gall_item_w * $rows) + ($rows * $pad) + ($rows * $border);
        ...
        $('.gallery-filmstrip').each(function () {
            $first_caption = $(this).find('.strip-container a:eq(0) img').attr('alt');
        ...
        function mk_filmstrip_gallery_size() {
            ...
            $backdrop = $filmstrip.find('.filmstrip-backdrop');
            $img_w = $backdrop.attr('data-img-width');
            $avail_space = $filmstrip.width();
            // subtract padding
        ...
        function filmstrip_select_pic($pic_link) {
            $fullpic = $pic_link.attr('fullpic');
            $caption = $pic_link.find('img').attr('alt');
            $filmstrip = $pic_link.closest('.gallery-filmstrip');
            $filmstrip.find('.filmstrip-current img').attr('src', $fullpic);
            $filmstrip.find('.filmstrip-caption').html($caption);
        }

        $filmstrip_hover = false;
        $filmstrip_click = false;
        ...
        function scroll_filmstrip_preview($strip) {
            ...
            $strip_pics = $strip.parent().find('.strip-pics');
            $num_pics = $strip_pics.find('img').size();
            $unit_w = 110; // 1 pic plus margin-right
            $full_w = $num_pics * $unit_w;
            $nav_w = 90; // next/prev arrows + margin
            $big_pic_w = $strip_pics.closest('.gallery-filmstrip').find('.filmstrip-current img').width();
            $view_w = $big_pic_w - $nav_w;

            // fiture out which direction we're going, and if we're out of bounds
            $cur_left = parseInt($strip_pics.css('left'));
            $direction = '-=';
            $out_of_bounds = false;
        ...
        $('.wp-caption').each(function () {
            if ($(this).parent().hasClass('splash') || $(this).parent().hasClass('quad')) {
                ...
                $title_div = '<div class="custom-caption-title">' + $title + '</div>';
                $text = $(this).find('p').html();
                $text_div = '<div class="custom-caption-text">' + $text + '</div>';
                $html = '<div class="custom-caption">' + $title_div + $text_div + '</div>';
                $(this).find('p').replaceWith($html);
                if ($(this).parent().hasClass('splash')) {
                    $(this).wrap('<div class="splash-container"></div>');
                }
                else if ($(this).parent().hasClass('quad')) {
                    // remove margin on every other (hopefully left-side) element
                    $('.custom-caption-container').filter(':odd').css('margin-right', 0);
                    $img = $(this).find('img');
                    $img.removeClass();
                    $('.custom-caption-title').addClass('pic-overlay');
                    $link = $(this).find('a').attr('href');
                    if ($link) {
                        $pic_overlay = $(this).find('.pic-overlay');
        ...
        $sub_items = $menus.find('li.sub-item');
        $sub_items.hide();

        // clicking on parent menu items (li, not link itself) toggles submenu items
        $parent_items = $menus.find('li.has-children');
        ...
        function show_menu_context($menu) {
            ...
            $curr_item = $menu.find('.current-menu-item');
            ...
            if ($curr_item.hasClass('menu-item-object-category')) {
                $curr_depth = $curr_item.attr('data-depth');
                if ($curr_depth > 0) {
            ...
            // expand ancestry
            $ancestors = $menu.find('.current-menu-ancestor');
        ...
        $('#sidebar').find('.widget_cat_loop_simple ul.post-loop-wrap').each(function () {
            $extras = $(this).find('li > *:not("p.post-title")');
            if ($extras.length == 0) {
        ...
        $is_math = location.href.substring(0, 12) == 'http://math.';
        $('.localist-event').each(function (index) { // for each event
            // duplicate event name & date so it can show in the rollover too
            $e_name = $(this).find('.lwn').clone().addClass('cal-rollover-top');
            $(this).find('.cal-rollover .lwi0').after($e_name);

            // date calculations
            $date_html = $(this).find('.lwn0:first');
            $date = jQuery.trim($date_html.text());
            $date_copy = $date; // save for later
            $date_bits = $date.split(' ');	// $date_bits = ["Sep", "25", "", "7pm"...]
            $mon = $date_bits[0].toLowerCase();
            $day = $date_bits[1].replace(',', '');
            if ($date_bits[2] != "") {
                $time = $date_bits[2];
            }
            else {
            ...
            $today_obj = new Date();
            $cur_year = $today_obj.getFullYear();
            $cur_mon = $today_obj.getMonth();
            $event_date_obj = new Date($mon + " " + $day + ", " + $cur_year);
            $event_mon = $event_date_obj.getMonth();
            ...
            $event_dow = $dow_names[$event_date_obj.getDay()];

            // move datetime to after title
            $roll_date = $(this).find('.cal-rollover-top span.lwn0').prepend($event_dow + ', ').remove();
            $(this).find('.cal-rollover-top').append($roll_date);

            // add month sprite
            $all_html = '<div class="localist-fancy-date">' +
            ...
            $more = '<a class="more" href="' + $url + '">[ more ]</a>';
            $(this).find('.cal-rollover-bot').append($more);

            // fix thumb image url - they started giving us dinky ones :<
            $thumb = $(this).find('.cal-rollover img.localist-thumb-img');
            $too_small = $thumb.attr('src');
            $bigger = $too_small.replace("/small/", "/big/");
...
define('common',['jquery', 'purl', 'jquery_ui'], function ($) {
    return common = {
        getPageUrl: function () {
            var $d_url   = $.url();
            var page_url = $d_url.attr('protocol') + '://' + $d_url.attr('host') + $d_url.attr('path');
            return {urlObj: $d_url, pageUrl: page_url};
        },
        ...
        appendAnchor: function ($el, url) {
            ...
            $anchor.tooltip({
                position: {my: "left top+20", at: "left bottom", collision: "flipfit", track: true}
            }).on('click', function (event) {
                event.preventDefault();
                event.stopPropagation();
                common.copyTextToClipboard(event, url);
            });
            $el.prepend($anchor);
        },
...
require(['jquery', 'featherlight_src', 'detect_swipe', 'jquery_ui', 'cookie'], function ($) {
    console.log('inside quicklinks.js');
    /*!
     * Case insensitive .contains()
     */
    jQuery.expr[":"].Contains = function (r, s, q) {
        return (r.textContent || r.innerText || "").toUpperCase().indexOf(q[3].toUpperCase()) >= 0
    };

    /*!
     * Prepare input fields for filtering
     */
    (function (q) {
        q.fn.setup_filter = function (r, t, s) {
            return this.each(function () {
                var w = q(this);
                if (typeof(w.data("placeholder")) === "undefined") {
                    w.data("placeholder", "Type to filter...")
                }
                var u = w.data("placeholder");
                var v = w.attr("name");
                w.val(function () {
                    var x = jQuery.url();
                    if (!x.attr("query").length || typeof(x.param(v)) === "undefined") {
                        return w.data("placeholder")
                    } else {
                        return x.param(v)
                    }
                }).blur(function () {
                ...
                q('<a class="clear-filter" href="javascript:void(0)"></a>').insertAfter(w).click(function (x) {
                    x.preventDefault();
                    w.val(u);
                ...
                q("#fancybox-loading").ajaxStart(function () {
                    q(this).show()
                }).ajaxStop(function () {
                    q(this).hide()
                });
                if (w.val() != w.data("placeholder")) {
                    r()
                }
            })
        }
     ...
     */
    (function (q) {
        q.support.touch = "ontouchend" in document;
        if (!q.support.touch) {
            return
        }
        var u = q.ui.mouse.prototype, s = u._mouseInit, r;

        function t(x, w) {
            if (x.originalEvent.touches.length > 1) {
                return
            }
            x.preventDefault();
            var v = x.originalEvent.changedTouches[0], y = document.createEvent("MouseEvents");
            y.initMouseEvent(w, true, true, window, 1, v.screenX, v.screenY, v.clientX, v.clientY, false, false, false, false, 0, null);
            x.target.dispatchEvent(y)
        }

        u._touchStart = function (v) {
            var w = this;
            if (r || !w._mouseCapture(v.originalEvent.changedTouches[0])) {
                return
            }
            r             = true;
            w._touchMoved = false;
        ...
        u._mouseInit  = function () {
            var v = this;
            v.element.bind("touchstart", q.proxy(v, "_touchStart")).bind("touchmove", q.proxy(v, "_touchMove")).bind("touchend", q.proxy(v, "_touchEnd"));
            s.call(v)
        }
    })(jQuery);
    ...
    // html constants
    $external_link = '<div class="sprite icon-16 external-link"></div>';
    $edit_link     = '<a class="sprite icon-16 edit"></a>';
    $delete_link   = '<a class="sprite icon-16 delete"></a>';
    ...
    $cookie_expires = new Date(2030, 10, 30);
    ...
    if ($('#quick-container').length > 0) {
        ...
        // if ipad, do full page
        if (!$('body.ipad').length) {
            $('.quick-launch').featherlightGallery(/*$fancy_args*/);
        }

        // Add google analytics event tracking to Landing Page links
        $('#your-links li a').on('click', function (e) {
            $label       = $(this).text();
            $url         = $(this).attr('href');
     ...
     *-----------------------------------------------------------------------------------------*/
        ...
        $unsaved_changes = false;  // unsaved to database, cookie is always automatic

        // initialize link list & set login message
        $status = $.cookie('quicklinks_login_status');
        ...
        if (!$links_loaded) {
            ql_prepopulate_links($('#quicklinks #your-links'), 'popup');
        }
        ql_user_info();
        ql_set_size();

        // create sortable/draggable lists
        $('#link-options li').draggable({
            connectToSortable: $('#your-links'),
            helper           : 'clone',
            revert           : 'invalid',
            //containment: $('#your-links')
            containment      : $('div.quick-content')
        });
        $("#link-options li").on("dragstop", function (event, ui) {
        ...
        $('#your-links').sortable({
            update: function (e, ui) {
                ...
                $has_delete = $(ui.item).has('a.delete').length;
                if ($has_delete == 0) {
        ...
        $('#quicklinks .quick-tool').click(function (e) {
            hide_tools();
            $tool = $(this).attr('tool');
            show_tool($tool);
            ql_set_size();
        ...
        // delete
        $('#your-links .delete').on('click', function (e) {
            $(this).parent().remove();
            ql_set_cookie();
            e.preventDefault();
        });
        ...
        $('#your-links a.edit').on('click', function (e) {
            // get link info
            $ql_item = $(this).parent().find('.quick-item');
            $title   = $ql_item.text();
            ...
            $form = custom_item_form($type, $title, $url);

            // hide all inline forms (can only have 1 open at a time)
            $('.inline-edit').hide();
            $('#your-links li').removeClass('editing');
        ...
        $('.custom-item-form').on('submit', function (e) {
            ...
            $attach = '';
            // get link data
            $title  = $(this).find('.custom-item-title').val();
        ...
        // load saved data
        $('#quicklinks a.login-form-load').click(function (e) {
    ...
    function custom_item_form($type, $title, $url) {
        ...
        $form = $('#custom-item-form-template').clone();
        // prepopulate values
        if ($title) {
    ...
    function save_custom_item($title, $url, $type, $attach) {
        ...
        // generate list item
        if ($type == 'inline' || $type == 'inline-cat') {
            $html = ql_generate_link($link_info, 'edit');
    ...
    function ql_prepopulate_links($target, $type, $saved_data) {
        ...
        if ($data == '' || $data == null) {
            ...
            // custom links
            $items = $data.split($pair_sep);
            for ($n = 0; $n <= $items.length; $n++) {
                if ($items[$n] == undefined || $items[$n] == '') {
                    break;
                }
                $item_info = $items[$n].split($name_val_sep);
                $html      = ql_generate_link($item_info, $type);
                $list += $html;
            }
            $target.html($list);
    ...
    function ql_get_user_links() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success : function (data) {
                $('#your-links li').remove();
                ql_prepopulate_links($('#your-links'), 'popup', data.links);
                ql_set_cookie();
            }
        });
    }
    ...
    function ql_user_info() {
        ...
        if ($user = $.cookie('quicklinks_user')) {
            ...
            $('.quick-tools .quick-save').addClass('autosave').attr('title', 'Changes are automatically saved while logged in.');
            show_tool('college-links');
        }
            ...
            $('.quick-tools .quick-save').removeClass('autosave').attr('title', 'Save links across all browsers.');
        }
    }
    ...
    function ql_set_size() {
        ...
        $iframe        = $($parent_doc).find('iframe.fancybox-iframe');
        $iframe_height = $iframe.height();
        $header_height = $('.quick-header').outerHeight();
        $user_height   = 0;
        ...
        $content_height = $('.quick-content').height();

        // calculate space inside content in each col that is not the list
        $above_height_l = $('.quick-content .left-col:not(".hidden") .above-list').outerHeight() + 25;
        $above_height_r = $('.quick-content .right-col .above-list').outerHeight() + 25;
...
require(['jquery', 'common'], function ($, common) {
    ...
    $('.accordion-tab').each(function (index) {
        ...
        if (!$this_accordion.data('isExpando') && index === 0) { // tabs
            openTab($li);
        }
...
define("accordion_tabs", function(){});
