...
!(function ( $, common ) {
  ...
  $( ".likert-choice" ).change( function () {
    ...
    $('.site-header .network-utility a').each(function (i) {
        $(this).click(function (e) {
            _gaq.push(['_trackEvent', 'Mega Menu', 'Click', $(this).text().trim() + ': ' + $(this).attr('href')]);
        });
    });
  ...
  /********************
   ...
   <figure class="quad-image portrait">
     <a href="http://www.local.williams.edu" title="Is A Place">
       <img src="http://image.jpg" alt="Blhahaao ahahiodh a!!">
       <div class="custom-caption">
         <div class="custom-caption-title">Is A Place</div>
         <div class="custom-caption-text">Blhahaao ahahiodh a!!</div>
       </div>
     </a>
   </figure>
  ...
  const doQuadContainer = () => {
    const $quads = document.querySelectorAll('.quad-image');
    /*const quadsArr = Array.from($quads).reduce( (acc, val) => {
      val.className === 'ratio_4_3';
    }, 0);*/

    //$quads.setAttribute('style', 'visibility: hidden');
    const $wrapper = document.createElement('div');
    $wrapper.setAttribute('class', 'quad-container');
    wrapAll($quads, $wrapper);
    //$quads.removeAttribute('style');
  };
  ...
  function show_menu_context( $menu ) {
    ...
    // current menu ancestor class not properly applied to category menu item parents, do it (note: this does not
    // work at all depths)
  ...
  $( '#content' ).find( 'a[href^=#]' ).not( '.filter a' ).click( function () { // don't target external or jquery mobile links
    ...
    $(window).load(function () {
        document.querySelector('.searchui-incontent #input_wms_navbox--1').autofocus = true;
        console.log("working");
    });
